...
namespace {
    ...
    public:
        ...
        void add_expected_call(Real r, Real val, Real deriv) {
            expected_calls[r] = {val, deriv};
        }
...
TEST(TestAngle3bTransformation, CorrectlyUsesCutoff) {
    ...
    EXPECT_EQ(desc.value.size(), 4);
    EXPECT_NEAR(desc.value[0], r01 + r02, 1e-9);
    EXPECT_NEAR(desc.value[1], (r01 - r02) * (r01 - r02), 1e-9);
    EXPECT_NEAR(desc.value[2], r12, 1e-9);
    EXPECT_NEAR(desc.value[3], val01 * val02, 1e-9);
