...
TEST(TestManyBodyGapComponent, RealPolycutoff) {
    ...
    auto kernel = SquaredExpKernel<1, 0>(1.0, std::array<double, 1>{1.0});
    ...
    double exp_arg = std::pow(1.5 - expected_val, 2.0);
    double expected_K = std::exp(-0.5 * exp_arg);
    EXPECT_NEAR(quantities.energy(0), expected_K, 1e-9);

    // Kernel Gradient:
    // gradK = K * (1.5 - 128/81)
    double gradK = expected_K * (1.5 - expected_val);
    ...
    double expected_virial_xx = -2.0 * expected_deriv * gradK;
...
TEST(TestManyBodyGapComponent, TabulationRealPolycutoff) {
    ...
    for (size_t i = 0; i < value_grid.data_flat.size(); ++i) {
        double rho = value_grid.getCoord({i})[0];
        double expected_K = std::exp(-0.5 * std::pow(1.5 - rho, 2));
