...
namespace jgap {
    ...
    double ThreeBodySE::covariance(const Vector3 &t1, const Vector3 &t2) {
        return _energyScaleSquared * exp(-(t1 - t2).square() * _inverse2SigmaSq);
    }

    // Invariant triplets
    Vector3 ThreeBodySE::gradient(const Vector3 &changingTriplet, const Vector3 &constTriplet) {
        return (constTriplet - changingTriplet) * ((2 * _inverse2SigmaSq) * covariance(changingTriplet, constTriplet));
    }
}
