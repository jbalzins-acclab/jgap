...
namespace jgap {
    ...
    protected:
        ...
        std::vector<Real> findCoefficients(std::vector<ValuePtr<GapComponent>>& gap_components,
    ...
    private:
        Matrix<ColumnMajor> formMatrixA(const std::vector<ValuePtr<GapComponent>>& gap_components,
                                        const std::vector<Atoms>& training_data,
                                        const std::vector<EnergyData>& energy_data,
                                        const std::vector<Regularization>& sigmas_inverse) const;

        static std::vector<Real> formVectorB(const std::vector<ValuePtr<GapComponent>>& gap_components,
                                             const std::vector<EnergyData>& energy_data,
                                             const std::vector<Regularization>& sigmas_inverse);

        void fillU_mm(size_t starting_row, size_t starting_col, const ValuePtr<GapComponent>& gap_component,
                      Matrix<ColumnMajor>& A) const;

        static void fillInverseSigmaK_nm(const std::vector<ValuePtr<GapComponent>>& gap_components, const Atoms& atoms,
