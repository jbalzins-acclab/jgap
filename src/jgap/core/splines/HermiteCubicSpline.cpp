...
namespace jgap {
    ...
    InterpolationResults<1> HermiteCubicSpline::interpolate(std::array<double, 1> pos) const {
        ...
        const double origin = table.origin[0];
        const double spacing = table.spacing[0];
        ...
        double value = table({i}) + b[i] * dx + c[i] * dx * dx + d[i] * dx * dx * dx;
        double derivative = b[i] + 2.0 * c[i] * dx + 3.0 * d[i] * dx * dx;

        return {value, {derivative}};
    }

    std::array<double, 1> HermiteCubicSpline::getCutoff() const {
        return {table.origin[0] + static_cast<double>(table.sizes[0] - 1) * table.spacing[0]};
    }
    ...
    void HermiteCubicSpline::init(const Grid<1>& table_in) {
        ...
        for (size_t i = 1; i < n - 1; ++i) {
            double delta_left = (table({i}) - table({i - 1})) * inverse_spacing;
            double delta_right = (table({i + 1}) - table({i})) * inverse_spacing;
        ...
        for (size_t i = 0; i < n - 1; ++i) {
            double delta = (table({i + 1}) - table({i})) * inverse_spacing;
    ...
    size_t HermiteCubicSpline::findInterval(double r) const {
        const double origin = table.origin[0];
        const double inverse_spacing = 1.0 / table.spacing[0];
        const size_t n = table.sizes[0];
