...
namespace jgap {
    ...
    ScaledRegularizationRules::ScaledRegularizationRules(
        const double energy_sigma_per_atom,
        const double force_component_sigma,
        const double virials_iso_sigma_per_atom,
        const double virials_aniso_sigmas_per_atom,
        const double force_scale,
        const double min_scale
    ) :
    ...
    Regularization ScaledRegularizationRules::determine(const Atoms& atoms) const {
        ...
        const double scale = std::max(min_scale, 1.0 + force_scale * max_force);
