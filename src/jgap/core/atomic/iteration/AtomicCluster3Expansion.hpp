...
namespace jgap {
    ...
    public:
        static constexpr size_t ClusterPermutationsAvailable = factorial(2u);

        AtomicCluster3Expansion(const Species3AtomicSorted& species_set,
                                ClusterPermutationMode mode = ClusterPermutationMode::Reduced)
            : species_set(species_set),
              mode(mode),
              factor((species_set.nodes[0] != species_set.nodes[1] || mode == ClusterPermutationMode::Reduced)
                ...
                if (sep_list2_it != neighbour_list.neighbours_per_atom[atom_index].end()) {
                    ...
                    if (species1 == species2) {
                        if (mode == ClusterPermutationMode::Reduced) {
                            ...
                            result.reserve(result.clusters.size() + sep_list1.size() * (sep_list2.size() - 1));
                            for (auto it1 = sep_list1.begin(); it1 != sep_list1.end(); ++it1) {
                                for (auto it2 = std::next(it1); it2 != sep_list2.end(); ++it2) {
                                    result.add(atom_index, std::array{*it1, *it2});
                                    result.add(atom_index, std::array{*it2, *it1});
                                }
                            }
                        }
                    } else {
