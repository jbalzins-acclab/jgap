    ...
    public:
        ...
        void extend(Species3AtomicSorted species_set, TransformationPtr transformation) {
            ...
            if (!transformation->isRotationallyInvariant()) {
                JGAP_LOG_AND_THROW("Transformation is not rotationally invariant and cannot be used with ThreeBodySum");
            }
                ...
                for (const auto& [species_set, transformation]: transformations) {
                    ...
                    expansion.forEach(atom_index, nl, [&](const Cluster3& cluster) {
                        ...
                        for (size_t dim = 0; dim < Dim; dim++) {
                            ...
                            contribution.grad_r1[dim] *= factor;
                            contribution.grad_r2[dim] *= factor;
                        }
