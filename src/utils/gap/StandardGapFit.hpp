...
namespace jgap {
    inline GapPotential standardGapFit(const std::vector<Atoms>& training_data, const StandardGapParams& params) {
        ...
        if (params.eam_n_sparse > 0) {
            ...
            potential.addComponents(EamPairFunction::createComponents(
        ...
        ZblPotential zbp_pot = params.zbl_dataset_file
            ? ZblPotential{*params.zbl_dataset_file, training_data}
