...
#include "experimental/transform/manybody/ThreeBodySum.hpp"
...
namespace jgap {
    ...
    ValuePtr<NBodyAggregator<Dim>> ThreeBodySumSerialization<Dim>::deserialize(const SerializationNode& node) const {
        if (node.readOptionalAttribute<std::string>("name") != "ThreeBodySum" ||
        ...
        for (const auto& group_name: transformations_group.getChildNames()) {
            ...
            auto species_encoded = transformation_group.readAttribute<std::string>("species_set");
