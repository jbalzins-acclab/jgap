...
#include "instructions/ExitInstruction.hpp"
...
namespace jgap {
    ...
    public:
        ...
        ProcessingPipeline(std::vector<std::shared_ptr<PipelineInstruction>> instructions,
            ...
            : instructions_(std::move(instructions)), state_(state) {
            instructions_.push_back(std::make_shared<ExitInstruction>());
        }

        void changeState(const PipelineState &state) { state_ = state; }
